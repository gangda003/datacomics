...
function toDo(what) {
  ...
  switch (what) {
    ...
    case 'capture_special_page':
      ...
      break;
    case 'capture_SVG':
      bg.screenshot.showSVGArea();
      window.close();
      break;
    case 'capture_HTMLTable':
      bg.screenshot.showHTMLTableArea();
      window.close();
      break;
  }
}
...
function init() {
  ...
  i18nReplace('captureWebpageText', 'capture_webpage');
   i18nReplace('captureSVGText', 'capture_SVG');
   i18nReplace('captureHTMLTableText', 'capture_HTMLTable');
    ...
    chrome.tabs.sendMessage(tab.id, {msg: 'is_page_capturable'},
      function(response) {
        ...
        if (response.msg == 'capturable') {
         ...
         // $('captureWebpageItem').style.display = 'block';
          //$('captureWebpageItem').style.display = 'none';
          $('captureSVGItem').style.display = 'block';
          $('captureHTMLTableItem').style.display = 'block';
          ...
          resizeDivWidth('captureSVGText', textWidth);
          resizeDivWidth('captureHTMLTableText', textWidth);
          var bg = chrome.extension.getBackgroundPage();
  ...
  $('captureSVGItem').addEventListener('click', function(e) {
    toDo('capture_SVG');
  });
   $('captureHTMLTableItem').addEventListener('click', function(e) {
    toDo('capture_HTMLTable');
  });
}
