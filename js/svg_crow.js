 ...
 doctype:'<?xml version="1.0" standalone="no"?><!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">',
	...
	initialize : function(d3) {
		...
		documents.forEach(function(document) {
			...
			var newSources = getSources(document, styles);
			//var newSources = getSources_new(document, styles);
		...
		function getSources_new(doc, styles) {
	    var svgInfo = [],
		...
		function getSources(doc, styles) {
			...
			svgs.each(function() {
				...
				svg.attr("version", "1.1").insert("defs", ":first-child").attr("class", "svg-crowbar").append("style").attr("type", "text/css");
				...
				svgInfo.push({
					...
					//source : [svg_crow.doctype + source]
					source : [source]
				});
			});
		...
		function returnSVG(source)
		{
			page.sendMessage({msg: 'svg_captured',svg:source.source});
...
/*
 ...
 script.src = "//cdnjs.cloudflare.com/ajax/libs/d3/3.4.2/d3.min.js";
